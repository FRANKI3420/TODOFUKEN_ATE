...
function saveRanking(accuracy, time) {
  let ranking = JSON.parse(localStorage.getItem("muraQuizRanking")) || [];
  ...
  localStorage.setItem("muraQuizRanking", JSON.stringify(ranking));
}
// --- ランキング削除機能 ---
document.getElementById("resetButton").addEventListener("click", () => {
  ...
  if (confirmDelete) {
    // LocalStorageのデータを削除
    localStorage.removeItem("muraQuizRanking");
...
function showRanking() {
  const ranking = JSON.parse(localStorage.getItem("muraQuizRanking")) || [];
